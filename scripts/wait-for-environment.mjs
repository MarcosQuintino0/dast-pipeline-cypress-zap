...
async function sondar(servico) {
  try {
    ...
    if (!resposta.ok)
      return { pronto: false, detalhe: `HTTP ${resposta.status}` };
    return servico.interpretar(await resposta.text());
  } catch (erro) {
    if (erro.name === "TimeoutError")
      return { pronto: false, detalhe: "timeout" };
    if (erro instanceof SyntaxError)
      return { pronto: false, detalhe: "resposta nao e JSON" };
...
function relatar(resultados) {
  for (const { servico, pronto, detalhe } of resultados) {
    process.stdout.write(
      `  ${pronto ? "OK   " : "FALHA"} ${servico.nome.padEnd(5)} ${detalhe}\n`,
    );
  }
}
...
async function main() {
  ...
  for (;;) {
    ...
    const nomes = pendentes
      .map((resultado) => resultado.servico.nome)
      .join(", ");
